...
// @ts-ignore
export default function EditAppointment({ appointmentID, defaultValues, doctors, patients }) {
    ...
    async function onSubmit() {
        ...
        const values = {...form.getValues(), appointmentID: appointmentID}
        const result = await editAppointment(values);
        if (result.success) {
            toast({description: (<div className={"flex items-center flex-row gap-3 text-green-500"}><BadgeCheck size={24}/><h1>Appointment updated successfully!</h1></div>)});
        }else {
            toast({description: (<div className={"flex items-center flex-row gap-3 text-red-500"}><BadgeAlert size={24}/><h1>Appointment can&apos;t be updated!</h1></div>)})
        }
    }
